    ...
    async.eachSeries(this.paths, function (p, eachCb) {
      glob(self.matchPattern, { cwd:p }, function (e, matches) {
        if (e) {
          ...
          async.eachLimit(matches, 50, function (match, matchCb) {
            var assetFilePath = path.join(p, match);
            var isManifest = path.extname(assetFilePath) === '.mf';
            // Make sure we don't inadvertently match a directory that isn't a manifest directory
            fs.stat(assetFilePath, function (err, stat) {
              if (err) {
                matchCb(err);
              } else {
                if (!stat.isDirectory() || isManifest) {
