      ...
      async.forEach(assetBundle.getAllAssets(), function (asset, eachCb) {
        async.waterfall([
          ...
          function (wfCb) {
            ...
            try {
              _.each(self.lookFor, function (lf) {
                lf.lastIndex = 0; // Reset the regexp
                var result;
                while ((result = lf.exec(contents)) !== null) {
                  var logicalDepPath = result[1];
                  var depAsset = assetBundle.getAssetByLogicalPath(logicalDepPath);
                  if (depAsset) {
                    assetBundle.addDependency(asset.assetFilePath, depAsset.assetFilePath);
                  } else {
