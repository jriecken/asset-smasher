...
// TODO: find cyclical dependencies
...
ManifestWalker.prototype = {
  ...
  execute:function (assetBundle, cb) {
    var self = this;
    var assets = _.clone(assetBundle.assets);
    async.forEach(_.values(assets), function (asset, eachCb) {
  ...
  registerRequiredAsset:function (manifest, assetBundle, file, cb) {
    if (file) {
      if (!this.isFileInPaths(file)) {
        ...
        if (!assetBundle.assets[file]) {
          assetBundle.assets[file] = new Asset({
            assetBaseDir:this.pathForFile(file),
            assetFilePath:file
          });
        }
        // Add file to list of requires for the manifest
        var manifestFilePath = manifest.assetFilePath;
        var requires = assetBundle.requires[manifestFilePath];
        if (!requires) {
          requires = assetBundle.requires[manifestFilePath] = [];
        }
        if (!_.contains(requires, file)) {
          requires.push(file);
        }
        // If the required asset is also a manifest, recursively walk it
        if (path.extname(file) === '.mf') {
          this.walkManifest(assetBundle.assets[file], assetBundle, cb);
        } else {
          cb();
        }
      }
