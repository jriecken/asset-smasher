...
Snassets.prototype = {
  computeLogicalPaths: function(cb) {
    ...
    async.waterfall([
      ...
      function (b, wfCb) {
        try {
          ...
          var orderedAssets = _.map(bundle.getAssetOrder(), function(assetFilePath) {
        ...
        async.forEach(_.values(bundle.assets), function (asset, eachCb) {
          self.postProcessAsset(asset, eachCb);
        }, wfCb);
      },function(wfCb) {
        async.forEach(_.values(bundle.assets), function (asset, eachCb) {
          self.outputAsset(asset, eachCb);
        }, wfCb);
      }
    ], cb);
  },
  ...
  postProcessAsset: function (asset, cb) {
    this.phases.postCompilation.execute(asset, cb);
  },
  outputAsset:function (asset, cb) {
    this.phases.output.execute(asset, cb);
  },
